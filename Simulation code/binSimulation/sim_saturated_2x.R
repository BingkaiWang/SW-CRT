#########################################
# binary outcome
# Analysis with data generation process 2
# (smaller variance of beta_i for cluster-level intervention)
# (only include cluster-level random effect for glmer)
...
sim_saturated <- function(n, t, pplv, cpl, cpu, beta0, beta1, mu_bx, sd_cxc, sd_cxe, sdb, period_effect, dg_model, e_scale){
  ...
  fit_lme_ca <- lmer(Y ~ 0 +  period + trt_saturated + X1 + X2 + (1 | cluster_id) + (1 | cluster_period), 
  ...
  QQ <-     model.matrix(~0 +  period + trt_saturated + X1 + X2 , data = dfo)
  ...
  fit_gee <- glm(Y~0+period + trt_saturated + X1 + X2, data = dfo, family = "binomial")
  QQ <- model.matrix(~0 +  period + trt_saturated + X1 + X2, data = dfo)
  Qo <- map(1:n, function(i) QQ[dfo$cluster_id==as.character(i),] )
  ...
  if (e_scale == "rd"){
    ...
    fit_glm_ca <- glmer(Y ~ 0 +  period + trt_saturated + X1 + X2 + (1 | cluster_id), family = binomial, 
                        data = dfo)
    ...
    fit_geeglm <- geeglm(Y ~ 0 + period + trt_saturated + X1 + X2, data = dfo, id = cluster_id, family = "binomial", corstr = "exchangeable")
